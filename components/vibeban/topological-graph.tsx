...
export function computeLayers(
  ...
  for (const p of prerequisites) {
    if (!taskIdSet.has(p.task_id) || !taskIdSet.has(p.prerequisite_task_id))
      continue
...
export function TopologicalGraph({
  ...
  const { nodePositions, svgWidth, svgHeight, layerYPositions } =
    useMemo(() => {
      ...
      const totalContentHeight =
        layers.length * (NODE_HEIGHT + LAYER_GAP_Y) + PADDING_Y * 2
      ...
      layers.forEach((layer, layerIndex) => {
        const y =
          ...
          PADDING_Y -
          NODE_HEIGHT -
          layerIndex * (NODE_HEIGHT + LAYER_GAP_Y)
  ...
  useEffect(() => {
    if (
      ...
      layerYPositions[currentBank] !== undefined
    ) {
    ...
    return (
      <div className="flex flex-1 items-center justify-center text-gray-400 dark:text-gray-500">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm">Create tasks to see the vertical graph</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div
      ref={containerRef}
      className="relative min-h-0 w-full min-w-0 flex-1 overflow-auto scroll-smooth bg-white py-4 dark:bg-gray-900"
    >
      <svg
        width={svgWidth}
        height={svgHeight}
      ...
      >
        ...
        <g aria-hidden="true">
          {prerequisites.map((p) => {
            ...
            const isPrereqDone =
              tasks.find((t) => t.id === p.prerequisite_task_id)?.status ===
              'complete'
        ...
        {tasks.map((task) => {
          ...
          return (
            <g key={task.id}>
              ...
              >
                <div
                  className={`flex h-full w-full items-center gap-3 rounded-xl border px-3 shadow-sm ${
                    isDone
                ...
                >
                  ...
                  <div className="min-w-0 flex-1 py-2">
                    ...
                    <div
                      className={`truncate text-sm font-semibold leading-tight ${isDone ? 'text-gray-400 line-through' : 'text-gray-900 dark:text-white'}`}
                    >
                      {task.title}
                    </div>
                  </div>
